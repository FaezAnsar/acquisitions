...
const securityMiddleware = async(req, res, next) => {
    try {
        ...
        const client = aj.withRule(slidingWindow({mode:'LIVE',interval:1,max:limit, name:`${role}-limit`}));
        ...
        if (decision.isDenied()&& decision.reason.isShield()) {
            ...
            return res.status(403).json({ error: 'Forbidden',message:'Shield requests are not allowed' });
